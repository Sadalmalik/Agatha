...
public class NodeEditorWindowB : EditorWindow
{
	static NodeEditorWindowB window;
	...
	[MenuItem("Window/Node Editor B")]
	static void Init()
	{
		window              = (NodeEditorWindowB) EditorWindow.GetWindow(typeof(NodeEditorWindowB));
	...
	private void ShowNodes()
	{
		...
		_resizeHandle =
			AssetDatabase.LoadAssetAtPath("Assets/NodeEditor/Icons/ResizeHandle.png", typeof(Texture2D)) as Texture2D;
		_aaLine =
			AssetDatabase.LoadAssetAtPath("Assets/NodeEditor/Icons/AA1x5.png", typeof(Texture2D)) as Texture2D;
	...
	private void DrawNodeWindow(int id)
	{
		...
		if (!_handleActive && _action)
		{
			...
			{
				_handleActive =
					true;                                                           //active when cursor is in contact area
	...
	private void ResizeNode(int id, float deltaX, float deltaY)
	{
		...
		{
			case 1:
				if ((window1.width + deltaX) > _winMinX)
				{
					window1.xMax += deltaX;
				}

				if ((window1.height + deltaY) > _winMinY)
				{
					window1.yMax += deltaY;
				}
				...
				break;
			case 2:
				if ((window2.width + deltaX) > _winMinX)
				{
					window2.xMax += deltaX;
				}

				if ((window2.height + deltaY) > _winMinY)
				{
					window2.yMax += deltaY;
				}
